...
const AddInternPage = () => {
  ...
  const [allocatedInternships, setAllocatedInternships] = useState<string[]>([]);
  const [submittedIntern, setSubmittedIntern] = useState<any>(null);
  const [passwordFieldName, setPasswordFieldName] = useState(`pwd_${Math.random().toString(36).substring(7)}`);

  const [formData, setFormData] = useState(() => ({ ...ADD_INTERN_INITIAL }));
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await apiFetch(`/api/interns/check-email?email=${encodeURIComponent(formData.email)}`);
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await apiFetch(`/api/interns/check-enrollment?enrollmentNumber=${encodeURIComponent(formData.enrollmentNumber)}`);
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await apiFetch(`/api/interns/check-serial?serialNumber=${encodeURIComponent(formData.serialNumber)}`);
        if (res.ok) {
          const data = await res.json();
  ...
  const fetchDrafts = async () => {
    try {
      const res = await apiFetch("/api/interns/drafts");
      if (res.ok) {
        const data = await res.json();
        setDrafts(data);
      }
  ...
  const loadDraft = async (draftId: string) => {
    try {
      const res = await apiFetch(`/api/interns/drafts/${draftId}`);
      if (res.ok) {
        const data = await res.json();
        if (data) {
          setCurrentDraftId(draftId);
          setFormData(data.formData || { ...ADD_INTERN_INITIAL });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [courseRes, sessionRes, draftsRes, countryRes, categoriesRes, collegesRes] = await Promise.all([
          apiFetch("/api/courses/allot"),
          apiFetch("/api/academic/sessions"),
          apiFetch("/api/interns/drafts"),
        ...
        if (categoriesRes.ok) {
          ...
          setCategories(data.items?.map((cat: any) => cat.name) || []);
        }
        ...
        if (collegesRes.ok) {
          ...
          // Sort colleges A-Z by name
          const sortedColleges = Array.isArray(data)
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (step === 1 && (formData.fullName || formData.email || formData.internshipDomain)) {
        saveDraft(true);
      }
    }, 5000);
    return () => clearTimeout(timer);
  ...
  const filteredInternships = useMemo(() => {
    ...
    const internshipFees = useMemo(() => {
      if (!selectedInternshipDetails) return { internship: 0, discount: 0, total: 0 };
      const internship = selectedInternshipDetails.fees || 0;
      let total = internship;
      let discount = 0;
    ...
    }, [selectedInternshipDetails, couponInfo, referralInfo]);
  ...
  const confirmSubmit = async () => {
    // Explicitly check email exists again before submission to avoid race conditions with debouncing
    let emailAlreadyExists = emailExists;
    ...
    if (emailAlreadyExists) {
      toast.error("Email already exists as login ID");
      return;
    }
    if (enrollmentExists) {
    ...
    try {
      ...
      const payload = {
        ...
        referredByCode: formData.referralCodeUsed || undefined,
        appliedCoupon: formData.couponCode || undefined,
      };
      ...
      if (response.ok && data.success) {
        ...
        setSubmittedIntern(data.intern || data.data);
        setStep(3);
        setFormData({ ...ADD_INTERN_INITIAL });
        setSelectedInternshipDetails(null);
      } else {
        toast.error(data.message || "Failed to register intern");
      }
    } catch (error) {
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
      <div className="bg-primary/5 border border-primary/20 p-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t("Please review all information before final registration.")}</p>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-border pb-6">
          <div>
            <h1 className="font-heading font-extrabold text-3xl text-foreground uppercase tracking-tight flex items-center gap-3">
              ...
              {step === 1 ? t("Register New Intern") : t("Confirm Intern Details")}
            </h1>
            <p className="text-muted-foreground mt-1 text-sm font-medium uppercase tracking-wider">
              {step === 1
                ? t("Comprehensive intern registration for certificates and system access.")
                : t("Review the information below and confirm to register the intern.")}
            </p>
          </div>
        ...
        {step === 1 ? (
                        ...
                        <div className="md:col-span-3">
                          <div className="bg-primary/5 border border-primary/20 p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              {internshipFees.discount > 0 && (
                                <div className="space-y-1">
                                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">{t("Discount")}:</p>
                                  <p className="text-sm font-bold text-emerald-600">-₹{internshipFees.discount}</p>
                                </div>
                              )}
                              {internshipFees.discount > 0 && (
                                <div className="space-y-1">
                                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">{t("Total")}:</p>
                                  <p className="text-sm font-bold text-foreground">₹{internshipFees.total}</p>
                                </div>
                              )}
                            </div>
                          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-6 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="text-center space-y-2">
              <h2 className="text-3xl font-black uppercase tracking-tight text-foreground">{t("Registration Successful!")}</h2>
              <p className="text-muted-foreground font-bold uppercase tracking-wider text-xs">{t("Intern account has been created and verified.")}</p>
            </div>
