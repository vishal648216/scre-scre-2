...
pub async fn get_home_data(
    ...
    // 1. Check cache
    {
        let cache = HOME_DATA_CACHE.read().unwrap();
    ...
    let mut cat_cursor = cat_coll.find(doc! { "status": "Active" }, None).await.expect("Failed to fetch categories");
    ...
    let mut center_cursor = center_coll.find(doc! { "active": true }, None).await.expect("Failed to fetch centers");
    let mut centers = Vec::new();
    while let Some(res) = center_cursor.next().await {
        if let Ok(c) = res {
            centers.push(PublicCenter::from(c));
            if centers.len() >= 6 { break; }
        }
    }

    // Stats
    let user_coll = db.collection::<User>("users");
    let total_centers = user_coll.count_documents(doc! { "role": "center" }, None).await.unwrap_or(0);
    let total_students = user_coll.count_documents(doc! { "role": "student" }, None).await.unwrap_or(0);
    let total_courses = db.collection::<mongodb::bson::Document>("courses").count_documents(doc! { "status": "Active" }, None).await.unwrap_or(0);
    ...
    // 3. Update cache
    {
        let mut cache = HOME_DATA_CACHE.write().unwrap();
