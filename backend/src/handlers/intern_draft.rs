...
use crate::models::intern_draft::InternDraft;
...
pub async fn save_draft(
    ...
    println!(
        "DEBUG: intern_draft save_draft called by user: {}, role: {:?}, name: {:?}",
        claims.sub, claims.role, payload.name
    );

    if claims.role != UserRole::Admin && claims.role != UserRole::SuperAdmin {
        println!("DEBUG: intern_draft save_draft unauthorized role: {:?}", claims.role);
        return (
    ...
    let admin_id = match ObjectId::parse_str(&claims.sub) {
        ...
        Err(e) => {
            println!(
                "DEBUG: intern_draft save_draft invalid admin_id: {}, error: {}",
                claims.sub, e
            );
            return (
    ...
    let coll = db.collection::<InternDraft>("intern_drafts");
    let now = Utc::now();
    ...
    if let Some(mut draft) = existing {
        println!("DEBUG: intern_draft save_draft found existing draft: {:?}", draft.id);
        ...
        match coll.replace_one(doc! { "_id": id }, draft, None).await {
            Ok(_) => {
                println!(
                    "DEBUG: intern_draft save_draft replace_one success for id: {}",
                    id.to_hex()
                );
                (
                    StatusCode::OK,
            ...
            Err(e) => {
                println!("DEBUG: intern_draft save_draft replace_one failed: {}", e);
                (
        ...
        println!("DEBUG: intern_draft save_draft creating new draft");
        ...
        match coll.insert_one(draft, None).await {
            Ok(res) => {
                ...
                println!("DEBUG: intern_draft save_draft insert_one success, new id: {:?}", id);
                (
                    StatusCode::CREATED,
                    Json(DraftResponse {
                        success: true,
            ...
            Err(e) => {
                println!("DEBUG: intern_draft save_draft insert_one failed: {}", e);
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(DraftResponse {
                        success: false,
