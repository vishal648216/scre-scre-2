...
use crate::models::student_draft::StudentDraft;
...
pub async fn save_draft(
    ...
    println!(
        "DEBUG: student_draft save_draft called by user: {}, role: {:?}, name: {:?}",
        claims.sub, claims.role, payload.name
    );

    if claims.role != UserRole::Center {
        println!("DEBUG: student_draft save_draft unauthorized role: {:?}", claims.role);
        return (
    ...
    let center_id = match ObjectId::parse_str(&claims.sub) {
        ...
        Err(e) => {
            println!(
                "DEBUG: student_draft save_draft invalid center_id: {}, error: {}",
                claims.sub, e
            );
            return (
    ...
    let existing = if let Some(id_str) = &payload.id {
        ...
        // 2. Fallback to finding by name for this center
        coll.find_one(doc! { "center_id": center_id, "name": &payload.name }, None)
            .await
            .ok()
            .flatten()
    };
    ...
    if let Some(mut draft) = existing {
        println!("DEBUG: student_draft save_draft found existing draft: {:?}", draft.id);
        ...
        match coll.replace_one(doc! { "_id": id }, draft, None).await {
            Ok(_) => {
                println!(
                    "DEBUG: student_draft save_draft replace_one success for id: {}",
                    id.to_hex()
                );
                (
                    StatusCode::OK,
            ...
            Err(e) => {
                println!("DEBUG: student_draft save_draft replace_one failed: {}", e);
                (
        ...
        println!("DEBUG: student_draft save_draft creating new draft");
        ...
        match coll.insert_one(draft, None).await {
            Ok(res) => {
                ...
                println!("DEBUG: student_draft save_draft insert_one success, new id: {:?}", id);
                (
                    StatusCode::CREATED,
                    Json(DraftResponse {
                        success: true,
            ...
            Err(e) => {
                println!("DEBUG: student_draft save_draft insert_one failed: {}", e);
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(DraftResponse {
                        success: false,
...
pub async fn list_drafts(
    ...
    let coll = db.collection::<StudentDraft>("student_drafts");
    let filter = doc! { "center_id": center_id };
...
pub async fn get_draft(
    ...
    let coll = db.collection::<StudentDraft>("student_drafts");
    let filter = doc! { "_id": oid, "center_id": center_id };
